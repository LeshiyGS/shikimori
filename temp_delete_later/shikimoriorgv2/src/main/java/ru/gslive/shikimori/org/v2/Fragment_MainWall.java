...
public class Fragment_MainWall extends SherlockFragment {
 
	Boolean savetype = true, stop_update = true;
	int page_no = 1, iii=0;
	
	CommentsAdapter adapter_list;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
      ...
      //Основное окно
	    ...
	    if ((savedInstanceState != null)) {
	    	comments_sname.clear();
	    	comments_sname.addAll(savedInstanceState.getStringArrayList("comments_sname"));
			t_comments_sname.clear();
			t_comments_sname.addAll(savedInstanceState.getStringArrayList("t_comments_sname"));
			result.clear();
			result.addAll((ArrayList<SSDK_Comments>) savedInstanceState.getSerializable("result"));
			t_result.clear();
			t_result.addAll((ArrayList<SSDK_Comments>) savedInstanceState.getSerializable("t_result"));
	...
	public boolean onContextItemSelected(MenuItem item) {
	    ...
	    Intent intent = Functions.choiseContextItem(item, result.get(info.position-1), "user_page",
	    								Functions.preference.user_id, getActivity().getBaseContext(), lv_main);
	    if (intent != null){
  	...
  	private class AsyncLoading extends AsyncTask<Void, Void, Void> {
  	  ...
  	  protected void onPostExecute(Void result1) {
  		...
  		protected Void doInBackground(Void... arg0) {
  			try {
				t_result = SSDK_API.getComments_new(Functions.preference.user_id, "User", 20, page_no, Functions.preference.kawai);
				t_comments_sname.clear();
