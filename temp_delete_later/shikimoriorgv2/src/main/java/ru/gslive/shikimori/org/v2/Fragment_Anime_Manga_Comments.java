...
public class Fragment_Anime_Manga_Comments extends SherlockFragment {
		...
		public Object getItem(int position) {
		    ...
		    public View getView(final int position, View convertView, ViewGroup parent) {
			    ...
			    if (convertView == null) {
                    ...
                    holder.review = (TextView) convertView.findViewById(R.id.lbl_review);
	...
	public boolean onContextItemSelected(MenuItem item) {
	    ...
	    /*Intent intent = Functions.choiseContextItem(item, result.get(info.position-1), "comment",
	    					Fragment_Anime_Manga_Info.thread_id, getActivity().getBaseContext(), lv_main);
	    if (intent != null){
    ...
    //Асинхронная задача подгрузки новостей
  	private class AsyncLoading extends AsyncTask<Void, Void, Void> {
  		...
  		protected Void doInBackground(Void... arg0) {
  			try {
  				t_comments_sname.clear();
				t_result = SSDK_API.getComments_new(Fragment_Anime_Manga_Info.thread_id, "Entry", 20, page_no, Functions.preference.kawai);
