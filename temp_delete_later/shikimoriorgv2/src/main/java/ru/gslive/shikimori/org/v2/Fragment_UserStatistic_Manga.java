...
public class Fragment_UserStatistic_Manga extends SherlockFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
    	...
    	if (Functions.preference.theme.equals("ligth")){
    		view = inflater.inflate(R.layout.w_fragment_user_statistic_manga, container, false);
  	  	}else{
  	  		view = inflater.inflate(R.layout.d_fragment_user_statistic_manga, container, false);
  	  	}	
	...
	static void setStatistic(Context context){
  		...
  		//
  		if (Activity_UserStat.result.manga_scores.size() != 0) {
	  		int smax = 0;
	  		LinearLayout sll1 = new LinearLayout(context);
	  		LinearLayout sll2 = new LinearLayout(context);
	  		for (int i = 0; i < Activity_UserStat.result.manga_scores.size(); i++){
	  			if (smax < Activity_UserStat.result.manga_scores.get(i)){
	  				smax = Activity_UserStat.result.manga_scores.get(i);
	  			}
	  		}
	  		Log.d("size", "->"+Activity_UserStat.result.manga_scores.size());
	  		...
	  		for (int i = 0; i < Activity_UserStat.result.manga_scores.size(); i++){
				...
				tv.setText(Activity_UserStat.result.manga_scores_name.get(i));
				if (Functions.preference.theme.equals("ligth")){
				...
				pb.setMax(smax);
				pb.setProgress(Activity_UserStat.result.manga_scores.get(i));
  		...
  		if (Activity_UserStat.result.manga_scores.size() != 0) {
	  		...
	  		for (int i = 0; i < Activity_UserStat.result.manga_types.size(); i++){
	  			if (tmax < Activity_UserStat.result.manga_types.get(i)){
	  				tmax = Activity_UserStat.result.manga_types.get(i);
	  			}
	  		}
			...
			for (int i = 0; i < Activity_UserStat.result.manga_types.size(); i++){
				...
				TextView tv = new TextView(context);
				tv.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, (int)(metrics.density*20)));
				tv.setText(Activity_UserStat.result.manga_types_name.get(i));
				if (Functions.preference.theme.equals("ligth")){
					tv.setTextColor(context.getResources().getColor(R.color.black));
		  	  	}else{
		  	  		tv.setTextColor(context.getResources().getColor(R.color.gray_ligth));
				...
				pb.setMax(tmax);
				pb.setProgress(Activity_UserStat.result.manga_types.get(i));
				pb.setPadding(0, 2, 0, 2);
				pb.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, (int)(metrics.density*20)));
  		...
  		if (Activity_UserStat.result.manga_scores.size() != 0) {
	  		...
	  		for (int i = 0; i < Activity_UserStat.result.manga_ratings.size(); i++){
	  			if (rmax < Activity_UserStat.result.manga_ratings.get(i)){
	  				rmax = Activity_UserStat.result.manga_ratings.get(i);
	  			}
	  		}
			...
			for (int i = 0; i < Activity_UserStat.result.manga_ratings.size(); i++){
				...
				TextView tv = new TextView(context);
				tv.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, (int)(metrics.density*20)));
				tv.setText(Activity_UserStat.result.manga_ratings_name.get(i));
				if (Functions.preference.theme.equals("ligth")){
					tv.setTextColor(context.getResources().getColor(R.color.black));
		  	  	}else{
		  	  		tv.setTextColor(context.getResources().getColor(R.color.gray_ligth));
				...
				pb.setMax(rmax);
				pb.setProgress(Activity_UserStat.result.manga_ratings.get(i));
				pb.setPadding(0, 2, 0, 2);
				pb.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, (int)(metrics.density*20)));
  		...
  		tv_genres.setText(Html.fromHtml(Activity_UserStat.result.mgenres));
  		tv_studios.setText(Html.fromHtml(Activity_UserStat.result.publishers));
