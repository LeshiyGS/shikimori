...
public class Activity_Character extends ShikiSherlockFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if (t_target_id == null){
        	t_target_id = getIntent().getData().toString().substring(getIntent().getData().toString().lastIndexOf("/characters/") + 12, getIntent().getData().toString().lastIndexOf("-"));
        }
        target_id = t_target_id;
        
		//Подключение и выбор варианта меню.
        if (Functions.preference.theme.equals("ligth")){
            mDrawer = Functions.setMenuDrawer(actionbar, "Персонаж", R.layout.w_activity_main_profile, getWindowManager().getDefaultDisplay().getRotation(),
                    getBaseContext(), this);
        }else{
            mDrawer = Functions.setMenuDrawer(actionbar, "Персонаж", R.layout.d_activity_main_profile, getWindowManager().getDefaultDisplay().getRotation(),
                    getBaseContext(), this);
        }
        ...
        Adapter_Character viewpageradapter = new Adapter_Character(fm);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
  		...
  		if (requestCode == 100) {
  			if (resultCode == 1) {
  				Fragment_Character_Comments.lv_main.setRefreshing(true);
  			}
  		}
  	}
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
  		Intent intent_add = new Intent(Activity_Character.this, Activity_Add_Edit_Input.class);
  		switch (item.getItemId()) {
          	...
          	case 10:
    			...
    			intent_add.putExtra("commentable_id", Fragment_Character_Info.result.thread_id);
		...
		protected Void doInBackground(Void... arg0) {
			...
			try {
				...
				if (res.statusCode() == 200){
					Fragment_Character_Menu.in_favourites = false;
  				}
			} catch (IOException e) {
